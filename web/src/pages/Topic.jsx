    ...
    <div>
      ...
      <header className="mb-5">
        <div className="mb-1.5 flex flex-wrap items-center gap-2 text-[11px] font-bold">
          ...
          {topic.ap ? (
            <span className="rounded bg-brand-100 px-1.5 py-0.5 text-brand-800 dark:bg-brand-900/40 dark:text-brand-200">
              ANDHRA PRADESH
            </span>
          ) : null}
          <span className="rounded bg-slate-100 px-1.5 py-0.5 uppercase text-slate-600 dark:bg-slate-700 dark:text-slate-300">
            {topic.kind}
          </span>
        </div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">{topic.name}</h1>
        {topic.summary ? (
          <p className="mt-2 max-w-3xl text-sm text-slate-600 dark:text-slate-400">{topic.summary}</p>
        ) : null}
      </header>
      ...
      <section className="mb-6 rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800">
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          {papers.length ? (
            papers.map((p) => (
              <span
                key={p}
                className="rounded bg-brand-50 px-2 py-1 text-sm font-semibold text-brand-800 dark:bg-brand-900/30 dark:text-brand-200"
              >
                {p}
              </span>
            ))
        ...
        {units.length ? (
          <ul className="mt-3 space-y-0.5 text-xs text-slate-600 dark:text-slate-400">
      ...
      <section className="mb-6">
        ...
        {evidence.length ? (
          <ul className="space-y-1.5">
            {evidence.map((e, i) => (
              <li
                key={i}
                className="rounded-md border border-slate-200 bg-white p-2.5 text-sm dark:border-slate-700 dark:bg-slate-800"
              >
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {e.is_primary ? (
                    <span className="text-[11px] font-bold uppercase text-green-700 dark:text-green-400">
                      study from here
                    </span>
                  ) : null}
                ...
                {e.evidence ? (
                  <p className="mt-1 text-slate-700 dark:text-slate-300">{e.evidence}</p>
                ) : null}
              </li>
            ))}
          </ul>
        ) : (
          <p className="rounded-md bg-slate-50 p-3 text-sm text-slate-600 dark:bg-slate-800 dark:text-slate-400">
        ...
        {formats && formats.length ? (
          <div className="mt-3 rounded-md bg-slate-50 p-3 dark:bg-slate-800">
            ...
            <ul className="mt-1.5 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-700 dark:text-slate-300">
              {formats.map((f) => (
                <li key={f.format}>
          ...
          <ul className="flex flex-wrap gap-1.5">
            {related.map((r) => (
              <li key={r.slug + r.relation}>
                <Link
                  to={`/topics/${r.slug}`}
                  className="inline-block rounded-full border border-slate-300 px-3 py-1 text-xs text-slate-700 hover:border-brand-400 dark:border-slate-600 dark:text-slate-300"
                >
                  {r.name}
        ...
        {rows.map((it) => (
          <li key={it.id}>
            <Link
              to={`/item/${it.id}`}
              className={
                'block rounded-md border border-slate-200 bg-white p-2.5 hover:border-brand-400 dark:border-slate-700 dark:bg-slate-800 ' +
                (muted ? 'opacity-80' : '')
              }
            >
              <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
                <span>{it.date}</span>
                <span className="rounded bg-slate-100 px-1.5 py-0.5 uppercase dark:bg-slate-700">
                  {it.bucket}
                </span>
                {it.g1_bank ? (
                  <span className="rounded bg-green-100 px-1.5 py-0.5 font-bold text-green-800 dark:bg-green-900/40 dark:text-green-200">
                    {it.g1_bank}
                  </span>
                ) : null}
              </div>
