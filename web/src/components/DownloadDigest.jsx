...
// One day's digest, as a markdown file.
//
...
// export is for is the thing the app cannot do, which is travel: a vault, a
// phone with no signal, a printout the night before, a file that is still there
...
export default function DownloadDigest({ date, className = '', label = 'Download .md' }) {
  ...
  async function run() {
    ...
    setError('');
    try {
      await download(`/days/${date}/export.md`, `appsc-current-affairs-${date}.md`);
    } catch (err) {
  ...
  return (
    <span className="inline-flex flex-wrap items-center gap-2">
      <button
        ...
        onClick={run}
        disabled={busy}
        title={`Download ${date} as a markdown file — notes, static background, prelims facts, questions and answer key`}
