...
router.get('/flags', (req, res) => {
  ...
  const flags = db
    .prepare(
      `SELECT f.id, f.reason, f.note, f.status, f.created_at,
              u.name AS student_name, u.email AS student_email,
              m.id AS mcq_id, m.question, m.option_a, m.option_b, m.option_c, m.option_d,
              m.correct_option, m.explanation, m.format, m.fact_as_of,
              i.id AS item_id, i.headline, d.date AS day_date
         FROM ca_mcq_flags f
         JOIN users u ON u.id = f.user_id
         JOIN ca_mcqs m ON m.id = f.mcq_id
...
router.put('/flags/:id', (req, res) => {
  ...
  db.prepare(
    `UPDATE ca_mcq_flags SET status = ?, resolved_at = CASE WHEN ? = 'open' THEN NULL ELSE datetime('now') END
      WHERE id = ?`
...
router.get('/runs', (req, res) => {
  const runs = db.prepare('SELECT * FROM ca_runs ORDER BY created_at DESC LIMIT 50').all();
  res.json({ runs });
});

router.get('/runs/:id', (req, res) => {
  const run = db.prepare('SELECT * FROM ca_runs WHERE id = ?').get(req.params.id);
...
router.get('/corrections', (req, res) => {
  res.json({ corrections: db.prepare('SELECT * FROM ref_corrections ORDER BY id').all() });
});

router.post('/corrections', (req, res) => {
  const { topic, superseded_claim, correct_position, effective_date, match_terms } = req.body || {};
  if (!String(topic || '').trim() || !String(correct_position || '').trim()) {
    return res.status(400).json({ error: 'Topic and the correct position are required.' });
  }
  const info = db
    .prepare(
    ...
    .run(
      String(topic).trim(),
      String(superseded_claim || ''),
      String(correct_position).trim(),
      effective_date || null,
      String(match_terms || '')
    );
  res.json({ id: info.lastInsertRowid });
...
router.delete('/corrections/:id', (req, res) => {
  db.prepare('DELETE FROM ref_corrections WHERE id = ?').run(req.params.id);
  res.json({ ok: true });
});
